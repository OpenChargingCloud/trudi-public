...
namespace IVU.Common.Tls.RecordLayer
{
    ...
    {
        private static readonly ILog Log = LogProvider.For<RecordHandler>();
        ...
        /// <summary>
        /// Creates and Parses TLS Records
        /// </summary>
        /// <param name="version">Minimum TLS Version.</param>
        /// <param name="isClient">Use as TLS client [true] or server [false].</param>
        ...
        {
            Log.Debug("SetCipherSuite: " + cipherSuite.CipherSuiteName);
        ...
        public void ChangeLocalState()
        {
            ...
            if (_isClient)
            {
                ...
                if (_outputKeyBlock.ClientWriteMACKey != null)
                {
                    Log.Trace("MAC write key: " + BitConverter.ToString(_outputKeyBlock.ClientWriteMACKey));
                }
                ...
                if (_outputKeyBlock.ClientWriteKey != null)
                {
                    Log.Trace("AES write key: " + BitConverter.ToString(_outputKeyBlock.ClientWriteKey));
                    Log.Trace("AES write IV: " + BitConverter.ToString(_outputKeyBlock.ClientWriteIV));
                }
        ...
        private static void EncryptRecord(CipherSuite cipherSuite, Record record, ICryptoTransform cipher, byte[] nonceExplicit)
        {
            ...
            Log.Trace("Encrypted fragment: " + BitConverter.ToString(record.Fragment));
        }
                                                            ...
                                                            byte[] key, byte[] fixedIV, UInt64 seqNum)
        {
            ...
            byte[] additional = GetAdditionalBytes(seqNum, (byte)record.Type, record.Version, compressedLength);
