namespace TRuDI.Models
{
    ...
    public class ServerId
    {
        ...
        public ServerId(string serverId)
        {
            ...
            match = Regex.Match(serverId.ToUpperInvariant(), "^01[0-9A-F]{4}[0-9]{8}[0-9A-F]{4}$");
            if (match.Success)
            {
                // 0  2    6        14 16
                ...
                this.Medium = this.ConvertMBusMediumToObisMedium(byte.Parse(serverId.Substring(16, 2), NumberStyles.HexNumber, CultureInfo.InvariantCulture));
                this.FlagId = this.ConvertMBusManufacturerId(
                    byte.Parse(serverId.Substring(2, 2), NumberStyles.HexNumber, CultureInfo.InvariantCulture),
                    byte.Parse(serverId.Substring(4, 2), NumberStyles.HexNumber, CultureInfo.InvariantCulture));
                this.Number = uint.Parse(serverId.Substring(6, 8), NumberStyles.HexNumber, CultureInfo.InvariantCulture);
                this.Number = uint.Parse(
                    ((uint)((this.Number & 0xFF) << 24) | ((this.Number & 0xFF00) << 8)
                     | ((this.Number & 0xFF0000) >> 8) | ((this.Number & 0xFF000000) >> 24)).ToString("X8"));
