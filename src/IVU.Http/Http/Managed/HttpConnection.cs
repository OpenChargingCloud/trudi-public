...
namespace IVU.Http.Headers.Managed
{
    ...
    using IVU.Http.Utils;
    using IVU.Http.Logging;

    internal sealed class HttpConnection : IDisposable
    {
        private readonly ILog logger = LogProvider.For<HttpConnection>();
            ...
            CancellationToken cancellationToken)
        {
            try
            {
                ...
                {
                    var requestHeader = Encoding.ASCII.GetString(this._writeBuffer, 0, this._writeOffset);
                    this.logger.Trace("HTTP Request: {0}", requestHeader);
                }
